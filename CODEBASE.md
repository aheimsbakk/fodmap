...
| Scripts                | `npm test` → `node --test "tests/*.test.js"` (bare `node --test tests/` fails on Node 26); `npm run format`; `npm run verify:sync`; build run directly (`node scripts/build/build.mjs <out-dir>`)                                                                                  |
