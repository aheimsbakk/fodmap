...
├── tests/
│   ├── build.test.js            # data and build pipeline: generated page, counts, note-button + marker contracts
...
│   │   └── lib/
│   │       ├── merge.js         # release listing + merge (oldest → newest) into per-section items with change markers
...
| `base.css`       | base       | minimal reset, `body` (white bg, system sans-serif, `#333`), heading families + uppercase, `ul` normalization, `li` bullet marker (default glyph from `--marker-default`) with the `data-change` marker swap (BLUEPRINT §6.6, §4.7), `li.item` `overflow-wrap: anywhere` (long tokens at 150 % scale)                                                                                                                                                    |
...
- `tests/build.test.js` covers the data and build pipeline (BLUEPRINT §13.4):
  ...
  carries the info glyphs and the marker glyphs (§4.7).
...
- `tests/merge.test.js` covers the release merge and change markers
  ...
  (no markers) rules. Fixtures are built in a temp directory at runtime,
  so the repository holds no test data.
